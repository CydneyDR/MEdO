<?php
session_start();
require_once 'db.php'; // DB connection lang, walang header!

if (isset($_SESSION['sat_logged_in'])) {
    header("Location: sat_index.php");
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("SELECT * FROM satellite_offices WHERE username = ?");
    $stmt->execute([$username]);
    $satellite = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($satellite && $satellite['password'] === $password) {
        if ($satellite['status'] === 'Active') {
            $_SESSION['sat_logged_in'] = true;
            $_SESSION['sat_id'] = $satellite['id'];
            $_SESSION['sat_name'] = $satellite['office_name'];

            // Dito pupunta pag tama ang login
            header("Location: sat_index.php");
            exit;
        } else {
            $error = "Your office account is currently " . $satellite['status'];
        }
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Satellite Focal Login - STTI System</title>
    <style>
...
<body>
    ...
    <div class="login-card">
        <div class="login-logos">
            <img src="logo/STTI_LOGO__1_-removebg-preview.png" alt="Taytay"
        ...
        <h2>Satellite Portal</h2>
        <p>Satellite Focal Access Login</p>
